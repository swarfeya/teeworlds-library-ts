...
class Huffman {
    ...
    construct_tree(frequencies: number[]) {
        var nodes_left_storage: HuffmanConstructNode[] = [/*HuffmanConstructNode() for _ in range(HUFFMAN_MAX_SYMBOLS)*/]
        for (var _ = 0; _ < HUFFMAN_MAX_SYMBOLS; _++) {
            nodes_left_storage.push(new HuffmanConstructNode())
        }
        var nodes_left: number[] = [/*None for _ in range(HUFFMAN_MAX_SYMBOLS)*/]
        for (var _ = 0; _ < HUFFMAN_MAX_SYMBOLS; _++)
    ...
    compress(inp_buffer: Buffer, start_index: number = 0, size: number = undefined) {
        var output: number[] = []
        var bits = 0
        var bitcount = 0
    ...
    decompress(inp_buffer: Buffer, size: int = undefined) {
        var bits = 0
        var bitcount = 0
